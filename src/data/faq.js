export const faqData = [
	{
		question: "What types of leather goods do you make?",
		answer:
			"PerfectFooties handcrafts a range of leather goods including shoes (oxfords, loafers, derbies, boots, and sandals), belts, bags, wallets, and accessories. We also take custom orders — if you have a specific piece in mind, place the order and we will discuss the details.",
	},
	{
		question: "What type of leather do you use?",
		answer:
			"We work primarily with quality full-grain and top-grain leather, selected for grain consistency, thickness, and durability. Full-grain is the highest grade — it retains the natural surface of the hide, develops a rich patina over time, and is the most durable. We do not use bonded leather or low-grade splits in any of our handcrafted pieces.",
	},
	{
		question: "How do I place a custom order?",
		answer:
			"Custom orders are placed directly through the app — browse our collections, select a product, and choose your preferred options (colour, size, design details). Once you submit your order, you will be connected to our team via WhatsApp or email to finalise the specifics, confirm feasibility, and receive a quote. Production begins once a deposit is confirmed.",
	},
	{
		question: "How long does production take?",
		answer:
			"Standard products typically take 10–14 days to produce before shipping. Custom orders may take longer depending on complexity; we will give you an estimated timeline when you confirm your order. We do not rush production because quality craftsmanship takes time.",
	},
	{
		question: "How do I care for my leather goods?",
		answer:
			"Wipe your leather goods with a dry cloth after use. Apply a quality leather conditioner every 2–3 months to keep the leather supple. Avoid prolonged exposure to direct sunlight, rain, or heat sources. For shoes, use matching shoe cream or wax polish regularly and store on wooden shoe trees. See our blog for a detailed leather care guide.",
	},
	{
		question: "Can I get a refund on my order?",
		answer:
			"We do not offer refunds once production has begun, as each piece is handcrafted specifically for you. However, if there is a defect or quality issue with your order, contact us within 48 hours of receiving it and we will arrange a repair, replacement, or resolution. Please review your order details carefully before confirming.",
	},
	{
		question: "Do you deliver worldwide?",
		answer:
			"Yes, you can place orders from anywhere. We deliver across Nigeria — Lagos deliveries are typically faster, and orders outside Lagos are scheduled based on location. We also ship internationally; final international delivery timelines and costs are confirmed via WhatsApp or email after order submission. The ordering process is still open for all locations, and we will follow up with a full shipping quote and delivery plan once details are confirmed.",
	},
];
