...
function GreetingBanner() {
  ...
  return (
    ...
    >
      <Typography sx={{ fontFamily, fontWeight: 700, fontSize: { xs: '1.4rem', md: '1.8rem' } }}>
        {greeting}
      </Typography>
      <Typography sx={{ fontFamily, fontSize: '0.95rem', opacity: 0.85, mt: 0.5 }}>
        {today}
      </Typography>
    </Box>
  );
...
function AppointmentCard({ appointment }) {
  const displayName = appointment.customerName || appointment.name || 'Customer';
  const initials = displayName
    ...
    .toUpperCase()
    .slice(0, 2);
  const imminent = isWithinHours(appointment.parsedDate, 24);
  const serviceName = appointment.items?.[0]?.name || 'Service';
  ...
  return (
    <Paper
      sx={{
        ...
        mb: 1.5,
        borderRadius: 2,
        borderLeft: `4px solid ${imminent ? '#d32f2f' : '#1976d2'}`,
        display: 'flex',
        alignItems: 'center',
        gap: 2,
      }}
    >
      ...
      <Box sx={{ flex: 1, minWidth: 0 }}>
        <Typography sx={{ fontFamily, fontWeight: 600, fontSize: '0.9rem' }} noWrap>
          {displayName}
        </Typography>
        <Typography sx={{ fontFamily, fontSize: '0.8rem', color: '#777' }} noWrap>
          {serviceName}
        </Typography>
      </Box>
      <Chip
        icon={<AccessTimeIcon sx={{ fontSize: '0.9rem' }} />}
        label={formatRelativeTime(appointment.parsedDate)}
        size="small"
        sx={{
          fontFamily,
          fontSize: '0.75rem',
          bgcolor: imminent ? '#ffebee' : '#e3f2fd',
          color: imminent ? '#d32f2f' : '#1565c0',
          flexShrink: 0,
        }}
      />
    </Paper>
  ...
  return (
    <Box>
        ...
        {statCards.map((s) => (
          <Grid item xs={6} sm={4} md={3} lg={2} key={s.title}>
            <StatCard
              ...
              icon={s.icon}
              gradient={s.gradient}
              onClick={onNavigate ? () => onNavigate(s.section) : undefined}
            />
          </Grid>
        ))}
      </Grid>
      ...
      <Grid container spacing={3}>
                      ...
                      <TableCell sx={{ fontFamily }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                          ...
                          <Box>
                            ...
                            <Typography sx={{ fontFamily, fontSize: '0.7rem', color: '#999' }}>
                              {o.id.slice(0, 8)}...
                            </Typography>
                          </Box>
                        </Box>
        ...
        {/* Right column: Quick Actions + Appointments + Low Stock */}
        <Grid item xs={12} md={4}>
          ...
          {upcomingAppointments.length > 0 ? (
            ...
            <Paper sx={{ p: 2.5, borderRadius: 2, mb: 2, textAlign: 'center' }}>
              <Typography sx={{ fontFamily, color: '#999', fontSize: '0.9rem' }}>
          ...
          <Box sx={{ mt: 2 }}>
            <SectionHeader
              ...
              onViewAll={onNavigate ? () => onNavigate('pressons') : undefined}
            />
            {lowStock.length > 0 ? (
              lowStock
                .slice(0, 5)
                .map((p, i) => <LowStockCard key={`${p.name}-${i}`} product={p} />)
            ) : (
